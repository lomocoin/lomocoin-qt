...
static CBigNum bnProofOfStakeLimit(~uint256(0) >> 32);
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // lomocoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
...
bool CTransaction::CheckTransaction() const
{
    ...
    for (int i = 0; i < vout.size(); i++)
    {
        ...
        // lomocoin: enforce minimum output amount
                        ...
                        bool* pfMissingInputs)
{
    ...
    // lomocoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// lomocoin: find block wanted by given orphan block
...
//
//    // lomocoin: subsidy is cut in half every 16x multiply of difficulty
...
//    return min(nSubsidy, MAX_MINT_PROOF_OF_WORK);
	...
	if(pindexBest->nHeight == 1){
		nSubsidy = 492000 * COIN;
	}else if(pindexBest->nHeight < 2000){
	    nSubsidy = 500000 * COIN;

	}else if(pindexBest->nHeight >= 2000 && pindexBest->nHeight < LAST_POW_BLOCK){
		nSubsidy = COIN;
	}
//	if (fDebug && GetBoolArg("-printcreation"))
...
static const int64 nTargetTimespan = 2 * 60 * 60;  // one week
...
// lomocoin: find last block index up to pindex
...
unsigned int static GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // lomocoin: target change every block
    // lomocoin: retarget with exponential moving toward target spacing
    CBigNum bnNew;
    ...
    if(!fProofOfStake && fTestNet){
    	nInterval = 7*24*60*60 / nTargetSpacing;
    }
...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    // lomocoin: should not enter safe mode for longer invalid chain
}
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // lomocoin: check transaction timestamp
        ...
        else
        {
            ...
            // lomocoin: enforce transaction fees for every block
            if (nTxFee < MIN_TX_FEE)
                return fBlock? DoS(100, error("ConnectInputs() : %s not paying required fee=%s, paid=%s", GetHash().ToString().substr(0,10).c_str(), FormatMoney(MIN_TX_FEE).c_str(), FormatMoney(nTxFee).c_str())) : false;
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // lomocoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // lomocoin: track money supply and mint amount info
    ...
    // lomocoin: fees are not collected by miners as in bitcoin
    // lomocoin: fees are destroyed to compensate the entire network
    if (fDebug && GetBoolArg("-printcreation"))
...
// lomocoin: total coin age spent in transaction, in the unit of coin-days.
...
// lomocoin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos)
{
    ...
    // lomocoin: compute chain trust score
    pindexNew->bnChainTrust = (pindexNew->pprev ? pindexNew->pprev->bnChainTrust : 0) + pindexNew->GetBlockTrust();

    // lomocoin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // lomocoin: record proof-of-stake hash value
    ...
    // lomocoin: compute stake modifier
...
bool CBlock::CheckBlock() const
{
    ...
    // lomocoin: only the second transaction can be the optional coinstake
    for (int i = 2; i < vtx.size(); i++)
        if (vtx[i].IsCoinStake())
            return DoS(100, error("CheckBlock() : coinstake in wrong position"));

    // lomocoin: coinbase output should be empty if proof-of-stake block
...
//                   FormatMoney(IsProofOfWork()? GetProofOfWorkReward(nBits) : 0).c_str()));
    ...
    // lomocoin: check block signature
...
bool CBlock::AcceptBlock()
{
    ...
    // lomocoin: check that the block satisfies synchronized checkpoint
    ...
    // lomocoin: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // lomocoin: verify hash target and signature of coinstake tx
    if (pblock->IsProofOfStake())
    {
        ...
        if (!CheckProofOfStake(pblock->vtx[1], pblock->nBits, hashProofOfStake))
        {
            ...
            // popcoin: ask for missing blocks
            if (pfrom)
    ...
    // lomocoin: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // lomocoin: getblocks may not obtain the ancestor block rejected
    ...
    // lomocoin: if responsible for sync-checkpoint send it
...
// lomocoin: entropy bit for stake modifier if chosen by modifier
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < (uint64)15000000 + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        ThreadSafeMessageBox(strMessage, "LoMoCoin", wxOK | wxICON_EXCLAMATION | wxMODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    if (fTestNet)
    {
        ...
        nStakeMinAge = 60 * 60 * 2; // test net min age is 30 minute
    ...
    printf("%s Network: genesis=0x%s nBitsLimit=0x%08x nBitsInitial=0x%08x nStakeMinAge=%d nCoinbaseMaturity=%d nModifierInterval=%d\n",
           fTestNet? "Test" : "LoMoCoin", hashGenesisBlock.ToString().substr(0, 20).c_str(), bnProofOfWorkLimit.GetCompact(), bnInitialHashTarget.GetCompact(), nStakeMinAge, nCoinbaseMaturity, nModifierInterval);
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "CNN 11-Apr-2016 Quantum computing: Game changer or security threat?";
        CTransaction txNew;

        txNew.nTime = 1460330000;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1460330000;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 2184431167;

        if (fTestNet)
        {
            block.nTime    = 1460330000;
            block.nNonce   = 123335855;
        }
        CBigNum bnTarget;
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x4d412c1de3c6e93c69a0530209b355ae8b479ba7ffef852c77ab84e86914c072"));
        block.print();
        ...
        // lomocoin: initialize synchronized checkpoint
        if (!Checkpoints::WriteSyncCheckpoint(hashGenesisBlock))
            return error("LoadBlockIndex() : failed to init sync checkpoint");

        // lomocoin: upgrade time set to zero if txdb initialized
        {
            CTxDB txdb;
    ...
    // lomocoin: if checkpoint master key changed must reset sync-checkpoint
    {
        CTxDB txdb;
        string strPubKey = "";
...
string GetWarnings(string strFor)
{
    ...
    // lomocoin: wallet lock warning for minting
    if (strMintWarning != "")
    {
        nPriority = 0;
    ...
    // lomocoin: should not enter safe mode for longer invalid chain
    // lomocoin: if sync-checkpoint is too old do not enter safe mode
    ...
    // lomocoin: if detected invalid checkpoint enter safe mode
    ...
    // lomocoin: if detected unmet upgrade requirement enter safe mode
    ...
    // Alerts
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CAlert)& item, mapAlerts)
        {
            ...
            if (alert.AppliesToMe() && alert.nPriority > nPriority)
            {
                ...
                if (nPriority > 1000)
                    strRPC = strStatusBar;  // lomocoin: safe mode for high alert
            }
        }
    }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        // lomocoin: record my external IP reported by peer
        ...
        // lomocoin: relay sync-checkpoint
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    if (inv.hash == pfrom->hashContinue)
                    {
                        ...
                        // lomocoin: best block is always the reference block to prevent
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // lomocoin: tell downloading node about the latest block if it's
...
CBlock* CreateNewBlock(CReserveKey& reservekey, CWallet* pwallet, bool fProofOfStake)
{
    ...
    // lomocoin: if coinstake available add coinstake tx
...
void BitcoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateBitcoins || fProofOfStake)
    {
        ...
        if (fProofOfStake)
        {
            // lomocoin: if proof-of-stake block found then process block
            if (pblock->IsProofOfStake())
            {
