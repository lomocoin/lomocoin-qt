...
// LoMoCoin message start (switch from Bitcoin's in v0.2)
static unsigned char pchMessageStartBitcoin[4] = { 0xa9, 0xbe, 0xc4, 0xd9 };
static unsigned char pchMessageStartLoMoCoin[4] = { 0xa6, 0xb8, 0xc9, 0xd5 };
...
void GetMessageStart(unsigned char pchMessageStart[], bool fPersistent)
{
    if (fTestNet)
        ...
        memcpy(pchMessageStart, (fPersistent || GetAdjustedTime() > nMessageStartSwitchTime)? pchMessageStartLoMoCoin : pchMessageStartBitcoin, sizeof(pchMessageStartLoMoCoin));
}
